...
export class AdminEmployeesComponent implements OnInit {
  ...
  get totalEmployeesCount(): number {
    return this.employees.length > 0 ? 256 : 0;
  }
  ...
  exportExcel(): void {
    if (this.filteredEmployees && this.filteredEmployees.length > 0) {
      ...
      this.excelExport.exportToExcel(dataToExport, 'Employees_List');
    } else {
  ...
  onView(emp: any): void {
    this.router.navigate(['/admin/employees', emp.id || emp.employeeId]);
  }

  onEdit(emp: any): void {
    this.router.navigate(['/admin/employees', emp.id || emp.employeeId, 'edit']);
  }
