...
@Component({
  ...
  template: `
    <app-page-header title="Edit Project" subtitle="Update project scope, priority, team lead assignment, and timelines" [breadcrumbs]="[{label:'Dashboard',link:'/dashboard'},{label:'Projects',link:'/admin/projects'},{label:'Edit Project'}]">
      <button class="btn btn--cancel" (click)="cancel()"><i class="fas fa-arrow-left"></i> Back to Projects</button>
    </app-page-header>

    <div class="form-card">
      <div class="form-header">
        <h3><i class="fas fa-edit"></i> Edit Project Profile</h3>
        <p>Modify the details below to update project configuration.</p>
      </div>
  ...
  styles: [`
    ...
    .form-group input, .form-group select, .form-group textarea { padding: 11px 14px; border-radius: 10px; border: 1px solid #cbd5e1; font-size: 14px; outline: none; transition: border-color 0.2s; }
...
export class AdminProjectEditComponent implements OnInit {
  ...
  save(): void {
    ...
    this.projectService.updateProject(this.projectId, payload).subscribe({
      ...
      error: (err) => this.toast.error(err?.error?.message || 'Failed to update project')
    });
  }
