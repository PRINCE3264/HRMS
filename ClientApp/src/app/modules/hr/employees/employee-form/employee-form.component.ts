// Auto-generated HR Employee Form Component (mirrors Admin version)
...
export class HrEmployeeFormComponent implements OnInit {
  ...
  loadLookups(): void {
    this.departmentService.getDepartments().subscribe({
      next: (data: any[]) => this.departments = data.map((d: any) => ({ id: d.id, name: d.name })),
      error: () => this.toast.error('Failed to load departments')
    });
    this.departmentService.getDesignations().subscribe({
      next: (data: any[]) => this.designations = data.map((d: any) => ({ id: d.id, title: d.title })),
      error: () => this.toast.error('Failed to load designations')
    });
    this.departmentService.getBranches().subscribe({
      next: (data: any[]) => this.branches = data.map((b: any) => ({ id: b.id, name: b.name })),
  ...
  onImageSelected(event: Event): void {
    ...
    this.employeeService.uploadAvatar(file).subscribe({
      ...
      error: (err: any) => this.toast.error(err?.error?.message || 'Failed to upload profile picture')
    });
    input.value = '';
  }

  removeImage(): void { this.employeeForm.patchValue({ avatar: '' }); }
      ...
      if (this.editingId) {
        this.employeeService.updateEmployee(this.editingId, payload).subscribe({
          next: () => {
            ...
            this.router.navigate(['/hr/employees', this.editingId]);
          },
