...
export class HrPayrollComponent implements OnInit {
  activeTab: 'payslips' | 'deductions' | 'bonuses' = 'payslips';
  ...
  private STORAGE_KEY = 'hrm_hr_payroll_records';
  ...
  switchTab(tab: 'payslips' | 'deductions' | 'bonuses'): void {
    this.activeTab = tab;
    this.clearSelection();
  }
  ...
  exportSelectedToExcel(): void {
    ...
    const fileName = selectedRows.length > 0 ? `Payroll_${this.activeTab}_Selected_${this.selectedMonth}` : `Payroll_${this.activeTab}_Full_${this.selectedMonth}`;
  ...
  goToSalaryStructures(): void {
    this.router.navigate(['/hr/payroll/salary-structures']);
  }

  goToReports(): void {
    this.router.navigate(['/hr/payroll/reports']);
  }
}
